...
1. 中国大陆直连网络下美国区评论 RSS 返回空 feed；已通过 GitHub Actions（US runner）
   定时采集 + JSON/CSV 导入解决，详见 `data/README.md`。
2. RSS 每排序最多约 500 条（10 页 x 50），总覆盖有限，报告中会说明样本边界。
3. 未配置 `LLM_API_KEY` 时运行确定性模式：主题占位命名、仅统计发现、
   需求/测试明确标注未生成；配置后自动启用模型驱动。
