...
def generate_requirements(
    ...
    *,
    ...
    evidence_by_finding = {f["id"]: f.get("evidence_review_ids", []) for f in supported}
    try:
...
def generate_test_cases(requirements: list[dict], llm: Optional[object] = None) -> tuple[list[dict], str]:
    ...
    review_ids_by_requirement = {r["code"]: r.get("review_ids", []) for r in requirements}
    allowed_reviews = {rid for ids in review_ids_by_requirement.values() for rid in ids}
    try:
